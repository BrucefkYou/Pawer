...
export function usePagination({ url = '', onDataChange, needSort = [{ way:'asc-ID', name:'ID由小到大'}] }) {
    // 存放fetch所有容器
    ...
    const sortedData = data.sort((a, b) => {
        ...
        if (typeof a[sortName] === 'number' && typeof b[sortName] === 'number') {
            return sortWay === 'asc' ? a[sortName] - b[sortName] : b[sortName] - a[sortName];
        } else {
            // 如果是字串類型使用中英文進行排序
            return sortWay === 'asc'
                ? a[sortName].localeCompare(b[sortName], ['zh', 'en'], { numeric: true, sensitivity: 'base' })
                : b[sortName].localeCompare(a[sortName], ['zh', 'en'], { numeric: true, sensitivity: 'base' });
        }
    });
