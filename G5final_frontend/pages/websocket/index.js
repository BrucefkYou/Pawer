...
export default function Index(props) {
    ...
    useEffect(() => {
        ...
        ws.onmessage = (res) => {
            ...
            setMessage((prevMessages) => [...prevMessages, { ...getmessage, from: getmessage.from }]);
        };
        //發生錯誤時
        ws.onerror = (error) => {
                        ...
                        <ul className='d-flex flex-column'>
                            {message.map((v, i) => (
                                v.from === memberID ? (
                                    ...
                                    <li key={i} className="text-start m-3 position-relative testfrom">
                                            <Image alt="avatar" src={`http://localhost:3005/pet/${callID}.webp`} width={50} height={50} className="avatarfrom" />
                                        {v.content}
                                    </li>
                                )
                            ))}
